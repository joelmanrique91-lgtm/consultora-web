export const site = {
  brand: {
    name: 'Pistacho Estudio',
    tagline: 'Gestión integral de comunicación y marketing institucional',
  },
  services: [
    {
      title: 'Consultoría técnica y estratégica',
      description: 'Diagnóstico y recomendaciones accionables.',
      when: 'Cuando necesitás claridad sobre el estado actual y el mejor camino a seguir.',
    },
    {
      title: 'Análisis y diseño de soluciones',
      description: 'Definición de enfoque, plan y entregables.',
      when: 'Cuando buscás ordenar objetivos, planificar acciones y definir entregables.',
    },
    {
      title: 'Acompañamiento en proyectos',
      description: 'Soporte durante ejecución y seguimiento.',
      when: 'Cuando necesitás sostén durante la implementación y el seguimiento de avances.',
    },
  ],
  valueProps: [
    'Enfoque integral de la comunicación, con conciencia de marca y responsabilidad social como ejes del posicionamiento institucional.',
    'Acompañamiento cercano a las organizaciones, trabajando junto a sus equipos internos en procesos de planificación y toma de decisiones.',
    'Definición de objetivos claros y seguimiento de las acciones para asegurar coherencia, continuidad y resultados a mediano y largo plazo.',
  ],
  cta: {
    text: 'Coordinar una reunión',
    href: '/contacto',
  },
  footer: {
    name: 'Pistacho Estudio',
    positioning: 'Comunicación institucional estratégica para pymes B2B / industria',
    email: 'hola@pistachoestudio.com',
    location: 'San Juan, Argentina',
    linkedin: null,
  },
};
